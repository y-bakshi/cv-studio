...
def inline_tex(node: dict) -> str:
    """Serialize a Tiptap text node without allowing raw TeX injection."""
...
def content_text(node: dict) -> str:
    return "".join(
        ...
        else content_text(child)
        for child in node.get("content", [])
    )


def document_to_tex(document: dict) -> str:
    """Convert the supported visual document subset into the approved template."""
    body: list[str] = []
    for node in document.get("content", []):
        ...
        if kind == "heading":
            ...
            heading = (
                rf"\begin{{center}}{{\Huge\textbf{{{text}}}}}\end{{center}}"
                if level == 1
                else rf"\section{{{text}}}"
            )
            body.append(heading)
        elif kind == "paragraph":
...
def initial_document(name: str = "YOUR NAME") -> dict:
    ...
    def heading(value: str, level: int = 2) -> dict:
        return {
            "type": "heading",
            "attrs": {"level": level, "id": os.urandom(5).hex()},
            "content": [text(value)],
        }
    ...
    return {
        "type": "doc",
        "content": [
            heading(name.upper(), 1),
            paragraph("City, State • email@example.com • linkedin.com/in/username"),
            heading("Summary"),
            paragraph("Write a concise professional summary grounded in your experience."),
            heading("Experience"),
            paragraph("Role — Company | Dates"),
            {
                "type": "bulletList",
                "attrs": {"id": os.urandom(5).hex()},
                "content": [{
                    "type": "listItem",
                    "content": [paragraph(
                        "Describe an achievement with evidence and measurable impact."
                    )],
                }],
            },
            heading("Education"),
            paragraph("Degree — University | Graduation date"),
            heading("Technical Skills"),
            paragraph("Languages: • Frameworks: • Cloud:"),
        ],
    }


def compile_tex(source: str, output_path: Path) -> tuple[bool, str]:
    """Compile source in an isolated temporary directory and copy the PDF out."""
