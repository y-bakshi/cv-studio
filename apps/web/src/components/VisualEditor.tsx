...
export default function VisualEditor({content,onChange,onSelection}:{content:Record<string,unknown>;onChange:(doc:Record<string,unknown>)=>void;onSelection:(text:string,from:number,to:number)=>void}){
  ...
  return <div className="visual-wrap" onClick={()=>setMenu(null)}>
    <div className="format-bar">
      <button className={editor.isActive('bold')?'on':''} onMouseDown={action(()=>editor.chain().focus().toggleBold().run())}><Bold size={15}/></button>
      <button className={editor.isActive('italic')?'on':''} onMouseDown={action(()=>editor.chain().focus().toggleItalic().run())}><Italic size={15}/></button>
      <button className={editor.isActive('highlight')?'on':''} onMouseDown={action(()=>editor.chain().focus().toggleHighlight().run())}><Highlighter size={15}/></button><i/>
      <button onMouseDown={action(()=>editor.chain().focus().toggleBulletList().run())}><List size={15}/></button>
      <button onMouseDown={action(()=>editor.chain().focus().toggleOrderedList().run())}><ListOrdered size={15}/></button><i/>
      <button onMouseDown={action(()=>editor.chain().focus().undo().run())}><Undo2 size={15}/></button>
      <button onMouseDown={action(()=>editor.chain().focus().redo().run())}><Redo2 size={15}/></button>
    </div>
